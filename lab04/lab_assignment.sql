...
-- Task 2.4
SELECT first_name, last_name, department, manager_id FROM employees WHERE manager_id IS NOT NULL AND department = 'IT';
...
SELECT p.project_name,
    SUM(a.hours_worked) AS total_hours FROM assignments a JOIN projects p ON a.project_id = p.project_id GROUP BY p.project_name;
...
-- Employees who are assigned to projects
SELECT DISTINCT e.employee_id, e.first_name, e.last_name FROM employees e JOIN assignments a ON e.employee_id = a.employee_id;

-- Task 6.1
SELECT e.employee_id, e.first_name, e.last_name FROM employees e WHERE EXISTS ( SELECT 1 FROM assignments a WHERE a.employee_id = e.employee_id);

-- Task 6.2
SELECT e.employee_id, e.first_name, e.last_name FROM employees e WHERE e.employee_id IN (SELECT a.employee_id FROM assignments a JOIN projects p ON a.project_id = p.project_id WHERE p.status = 'Active');

-- Task 6.3
SELECT first_name, last_name,salary FROM employees WHERE salary > ANY (SELECT salary FROM employees WHERE department = 'Sales');
...
SELECT
    ...
    GREATEST(AVG(e.salary), MAX(e.salary) - 5000) AS salary_compare_high,
    LEAST(AVG(e.salary), MIN(e.salary) + 2000) AS salary_compare_low
FROM employees e
